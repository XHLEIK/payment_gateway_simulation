...
- **Modular NestJS Architecture**: Logically separated into domain modules (`auth`, `wallets`, `transactions`, `payments`, `analytics`, `redis`).
...
- **Deadlock Prevention (Lock Ordering)**: P2P transfer requests sort sender and recipient user UUIDs alphabetically prior to locking rows, breaking any potential circular dependency.
- **Serializable Isolation**: Complex transitions (processing checkout, approving money requests) run in database transactions with `SERIALIZABLE` isolation.
- **Webhook HMAC Signature Validation**: Receives asynchronous payment callbacks from the mock gateway, verifying they were generated by the gateway via SHA-256 HMAC signature headers.
